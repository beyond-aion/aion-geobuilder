...
public class BrushLstLoader {
  ...
  public static final List<String> eventMeshes = new ArrayList<>();
  ...
  public void load(final String path) throws IOException {
    ...
    for (int i = 0; i < meshDataCount; i++) {
      ...
      if (eventType > 0) {
        if (!eventMeshes.contains(meshFileNames.get(entry.meshIdx))) {
          eventMeshes.add(meshFileNames.get(entry.meshIdx));
        }
      }
